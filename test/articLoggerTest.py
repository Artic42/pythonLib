...
def test11():
    ...
    test.testIfEqual(
        2, log1File.lineCount(), "Checking logging in MASK and out of MASK"
    )
    log.close()
...
def test12_15():
    ...
    test.testIfEqual(
        logger.INFO_MASK, log.getMask(), "Checking mask value is INFO_MASK"
    )
    log.setValueInMask(logger.DEBUG_MASK)
    test.testIfEqual(
        logger.INFO_MASK | logger.DEBUG_MASK,
        log.getMask(),
        "Checking mask value is INFO_MASK | DEBUG_MASK",
    )
    ...
    test.testIfEqual(
        logger.DEBUG_MASK, log.getMask(), "Checking mask value is DEBUG_MASK"
    )
...
def test16():
    ...
    test.testIfEqual(
        2,
        len(fileUtils.getFilesInDirectory("logs")),
        "Checking two log files are created",
    )
    log.close()
